...
function usePost({ postId }: { postId: string }) {
  const { data, isLoading, isValidating } = useSWR(['post', postId], () => getPost(postId));
  return { data, isLoading, isValidating };
}

function usePostList() {
  const { data, isLoading, isValidating } = useSWR('posts', getPostList);
  return { data, isLoading, isValidating };
}
...
const Post: FC = () => {
  ...
  const { data, isLoading, isValidating } = usePost({ postId });
  return (
    <div>
...
const PostList: FC = () => {
  const { data, isLoading, isValidating } = usePostList();
  return (
    <div>
